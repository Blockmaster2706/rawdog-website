...
function processLsCommand(commandArguments) {
    ...
    if (typeof directory === "object" && !directory["Could not find directory"]) {
        ...
        files = Object.entries(directory)
            ...
            const prefix = typeof content === "object" ? "dir&nbsp;&nbsp;" : "file ";
            return prefix + name;
        })
            .join("\n");
    }
...
function getFileSystemFromPath(path) {
    ...
    for (const part of parts) {
        ...
        if (!current[part])
            return { "Could not find directory": true }; // Return empty object instead of null
        current = current[part];
...
function autoCompletePath() {
    ...
    if (lastPart) {
        const matches = Object.keys(current).filter((key) => key.startsWith(lastPart));
